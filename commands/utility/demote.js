// Throughout Roleplay officers can get demoted and this command will 
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('demote')
        .setDescription('Use this command to demote an officer')
        ...
        try {
            ...
            if ( (await interaction.member.roles.cache.some(role => role.id === idList.commissionerRole)) || (await interaction.member.roles.cache.some(role => role.id === idList.highCommandRole)) || (await interaction.member.roles.cache.some(role => role.id === idList.ftoLeadRole)) || (await interaction.member.roles.cache.some(role => role.id === idList.governmentRole)) ) {
                await interaction.editReply({
                    content: "Demoting the officer", ephemeral: true,
                });
                ...
                if ( player.roles.cache.has(idList.BCSORole) ){
                    ...
                    await interaction.editReply({
                        content: "This person is not an officer, you can only demote employees!", ephemeral: true, 
                    });
                    return;
                }
                ...
                if ( department === "BCSO" ) {
                    ...
                    if ((currentRankS > 200) && (currentRankS < 300) ) {
                        const newRank = "300";
                        rankName = "Lieutenant";
                        await member.roles.remove(idList.BCSOMajorRole);
                    ...
                    if ((currentRankS > 200) && (currentRankS < 300) ) {
                        const newRank = "300";
                        rankName = "Lieutenant";
                        await member.roles.remove(idList.captainRole);
                ...
                const logEmbed = new EmbedBuilder()
                    .setDescription(`<@${interaction.user.id}> demoted <@${player.id}> to ${rankName} `)
            ...
            await interaction.editReply({
				content: `An error has occurred, please try again in a bit and if the problem persists contact a high command officer. Code: demote.`, ephemeral: true,
			});
            console.error(err);
        }
    }
